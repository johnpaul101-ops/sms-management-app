...
const SmsBower = () => {
  ...
  useEffect(() => {
    const fetchBalance = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${baseUrl}/smsbower/checkbalance`);
        const data = await response.json();

        if (!response.ok)
          throw new Error(
            `${response.status}: Failed fetching balance from SMSBower`,
          );
    ...
    window.addEventListener("smsBowerRefetchBalance", fetchBalance);
